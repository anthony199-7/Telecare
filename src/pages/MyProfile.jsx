  ...
  const loadUserProfileData = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Sync context with the image URL
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
        ...
        <div className="p-6 sm:p-10 bg-gradient-to-r from-teal-500 to-cyan-600 text-white flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
          ...
          <div className="relative">
            <img
              src={newImageFile ? URL.createObjectURL(newImageFile) : (userData.image || assets.profile_pic)}
              alt="Profile"
        ...
        <form onSubmit={handleProfileUpdate}>
          ...
          <div className="p-6 sm:p-10 grid md:grid-cols-2 gap-8">
            ...
            <DetailsCard title="Contact Information">
              <EditableDetailItem label="Name" name="name" value={userData.name} icon="👤" isEditing={isEditing} onChange={handleInputChange} required={true} />
              <DetailItem label="Email" value={userData.email} icon="📧" /> 
              <EditableDetailItem label="Phone" name="phone" value={userData.phone} icon="📞" isEditing={isEditing} onChange={handleInputChange} />
            </DetailsCard>
            
            {/* Personal Details Card */}
            <DetailsCard title="Personal Details">
              <EditableDetailItem label="Gender" name="gender" value={userData.gender} icon="🚻" isEditing={isEditing} onChange={handleInputChange} />
              <EditableDetailItem label="Date of Birth" name="dob" value={userData.dob} icon="📅" isEditing={isEditing} onChange={handleInputChange} type="date" />
            </DetailsCard>
            ...
            <div className="md:col-span-2">
              <DetailsCard title="Address">
                ...
                />
                <EditableDetailItem label="Line 2" name="line2" value={userData.address?.line2} icon="📍" isEditing={isEditing} onChange={handleInputChange} />
              </DetailsCard>
            </div>
          ...
          <div className="p-6 sm:p-10 pt-0 md:col-span-2 flex justify-end space-x-4">
              {isEditing ? (
                  <>
                      <button type="submit" className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition">Save Changes</button>
                      <button type="button" onClick={() => setIsEditing(false)} className="bg-gray-300 text-gray-800 px-6 py-2 rounded-lg hover:bg-gray-400 transition">Cancel</button>
                  </>
              ) : (
                  <button type="button" onClick={() => setIsEditing(true)} className="bg-teal-600 text-white px-6 py-2 rounded-lg hover:bg-teal-700 transition">Edit Profile</button>
              )}
          </div>
        </form>
...
const EditableDetailItem = ({ label, name, value, icon, isEditing, onChange, type = 'text', required }) => (
